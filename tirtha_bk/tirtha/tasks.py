...
from .celery import app, get_task_logger, crontab
...
DBCLEANUP_INTERVAL = crontab(minute=0, hour=0, day_of_week=0) # Every 1 week at 00:00 on Sunday
...
def post_save_contrib_imageops(contrib_id):
    ...
    # Check images
    cel_logger.info(f"post_save_contrib_imageops: Triggering ImageOps for contrib_id: {contrib_id}.")
    cel_logger.info(f"post_save_contrib_imageops: Checking images for contrib_id: {contrib_id}...")
    iops = ImageOps(contrib_id=contrib_id)
    iops.check_images()
    cel_logger.info(f"post_save_contrib_imageops: Finished checking images for contrib_id: {contrib_id}.")
    
    # Create mesh after MESHOPS_CONTRIB_DELAY hours
    cel_logger.info(f"post_save_contrib_imageops: Will trigger MeshOps for {contrib_id} after {MESHOPS_CONTRIB_DELAY} hours...")
    mo_runner_task.apply_async(args=(contrib_id,), countdown=MESHOPS_CONTRIB_DELAY * 60 * 60)
...
def mo_runner_task(contrib_id):
    ...
    """
    from .workers import prerun_check, mo_runner

    cel_logger.info(f"mo_runner_task: Triggering MeshOps for contrib_id: {contrib_id}.")
    cel_logger.info(f"mo_runner_task: Running prerun checks for contrib_id: {contrib_id}...")
