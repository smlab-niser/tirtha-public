...
os.environ["TF_FORCE_GPU_ALLOW_GROWTH"] = "true" # To force nsfw_detector model to occupy only necessary GPU memory
silence_tensorflow() # To suppress TF warnings

# Local imports
from tirtha.models import Mesh, Contribution, Image, ARK, Run
from .alicevision import AliceVision
from .utils import Logger
from .utilsark import noid_check_digit, generate_noid
...
class MeshOps:
    ...
    def __init__(
        ...
        cls.logger = Logger(
            ...
            name=f"{cls.__name__}_{runID[:8]}"
        )
        cls.logger.info(f"ID {meshID} has Verbose ID (VID) {self.meshVID}. Using VID for logging.")
        ...
        try:
            ...
            # CHECK: Test ain_bulk()
            self.images = Image.objects.in_bulk(self.imageUUIDs, field_name="ID").values()
            self.contributions = [image.contribution for image in self.images]
            self.contributors = [contribution.contributor for contribution in self.contributions]
            ...
            run.save()
            cls.logger.info(f"Set the `images` & `contributors` attributes of Run {runID}.")
        except Exception as e:
            self._handle_error(
                excep=e,
                caller="Fetching images & contributors"
            )
    ...
    def _update_mesh_status(self, status: str):
        ...
        self.logger.info(f"Updated mesh.status to '{status}' for mesh {self.meshStr}...")
    ...
    def _serialRunner(cls, cmd: str, log_file: Path):
        ...
        try:
            ...
            cls.logger.error(f"Error in command execution for {logger.name}. Check log file: {log_path}.")
    ...
    def run_aliceVision(self):
        ...
        try:
            self.logger.info(f"Running the aliceVision pipeline(s) for mesh {meshStr}...")
            ...
            self.logger.info(f"Finished running aliceVision pipeline for mesh {meshStr}.")
    ...
    def run_obj2gltf(self, options: Optional[dict]={"secure": "true"}):
        ...
        if not out.is_file():
            self._handle_error(
                FileNotFoundError(f"Output not produced for {out}."),
                "meshopt"
            )
    ...
    def run_meshopt(
        ...
        self.logger.info(f"Finished running meshopt (gltfpack) for mesh {self.meshStr}.")
    ...
    def run_cleanup(self):
        ...
        try:
            ...
            self.logger.info(f"Looking up & deleting errored-out runs for mesh {meshStr}...")
            runs = Run.objects.filter(mesh=self.mesh, status="Error").order_by("-ended_at")
            if len(runs) > 0:
                for run in runs:
                    runDir = STATIC / "models" / Path(run.directory)
                    self.logger.info(f"Run {run.ID} for mesh {meshStr} has errors. Deleting...")
                    shutil.rmtree(runDir) # Delete folder
                    run.delete() # Delete from DB
                    self.logger.info(f"Deleted run {run.ID} for mesh {meshStr}.")
            self.logger.info(f"Deleted {len(runs)} errored-out runs for mesh {meshStr}.")
            ...
            self.arkURL = f"models/{self.meshID}/published/{self.meshID}_{curr_runID}.glb"
            ...
            self.logger.info(f"Archiving run {curr_runID} for mesh {meshStr} to {arcDir}.")
            ...
            self.logger.info(f"Archived run {curr_runID} for mesh {meshStr} to {arcDir}.")
    ...
    def run_ark(self, ark_len=16):
        ...
        # Create metadata
        self.logger.info(f"Creating metadata for ARK for run {run.ID} for mesh {meshStr}...")
        def_commit = "This ARK was generated & is managed by Project Tirtha (https://tirtha.niser.ac.in). " + \
        ...
        self.logger.info(f"Created metadata for ARK for run {run.ID} for mesh {meshStr}...")
    ...
    def run_finalize(self):
        ...
        self.logger.info(f"Finished finalizing run {self.runID} for mesh {self.meshStr}.")
...
class ImageOps:
    ...
    def __init__(
        ...
        try:
            self.logger.info(f"Accessing DB to get the contribution object for Contribution ID, {contrib_id}...")
            self.contribution = Contribution.objects.get(ID=contrib_id)
        except Contribution.DoesNotExist as excep:
            self.logger.error(f"Contribution with ID {contrib_id} does not exist in the DB.", exc_info=True)
        ...
        self.logger.info(f"Found {self.size} images for contribution {self.contribution.ID}.")
        ...
        if NSFW_MODEL_DIRPATH is not None and not NSFW_MODEL_DIRPATH.exists():
            ...
            raise FileNotFoundError(f"nsfw_detector model not found at {NSFW_MODEL_DIRPATH}.")
    ...
    def check_images(self):
        ...
        for idx, img in enumerate(self.images):
            ...
            if not self.check_content_safety(img_path):
                _update_image(img, 'nsfw', 'NSFW content detected by local NSFW filter.')
                continue
            ...
            if dr < self.thresholds['DR']:
                _update_image(img, 'bad', f"FAIL -- DR: {dr:.4f}; Rejected by DR threshold: {self.thresholds['DR']}.")
                continue

            # CNR
            cnr = gray_img.std() ** 2 / gray_img.mean()
            if cnr < self.thresholds['CNR']:
                _update_image(img, 'bad', f"FAIL -- DR: {dr:.4f}, CNR: {cnr:.4f}; Rejected by CNR threshold: {self.thresholds['CNR']}.")
                continue
            ...
            _update_image(img, 'good', f"PASS -- DR: {dr:.4f}, CNR: {cnr:.4f}, MANIQA: {iqa_score:.4f}; Thresholds: {self.thresholds}.")
        ...
        Contribution.objects.filter(ID=self.contribution.ID).update( # Does not trigger signals
            processed=True,
...
def prerun_check(contrib_id):
    ...
    if images_count < MESHOPS_MIN_IMAGES:
        return False, f"Not enough images to process mesh. Only {images_count} good images found."
...
def mo_runner(contrib_id: str):
    ...
    cons.print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Triggered by {contrib.ID} for Mesh {meshVID} <=> {meshID}.")
    cons.print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Starting MeshOps on Mesh {meshVID} <=> {meshID}.")
    try:
        mO = MeshOps(
            meshID=meshID
        )
        ...
        mO._run_all()
        cons.print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: Finished MeshOps on {meshVID} <=> {meshID}.") # <----------------
    except Exception as e:
        cons.print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: ERROR encountered in MeshOps for {meshVID} <=> {meshID}!")
